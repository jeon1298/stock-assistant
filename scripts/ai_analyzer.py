...
RAW_DATA_FILE = os.path.join(DOCS_DIR, "raw_data.json")

KST = timezone(timedelta(hours=9))

ANALYST_SYSTEM_PROMPT = """
당신은 3명의 애널리스트로 구성된 주식 분석 팀입니다.
단기 스윙 트레이딩(2~5일) 전문이며 한국/미국 종목을 50:50 비율로 분석합니다.
입력 데이터: 1차 필터링된 스크리닝 결과 + 경제지표 + 최신 뉴스 + ETF 섹터 정보
...
- ETF/레버리지는 현재 경제지표와 뉴스 기반으로 섹터 선정
...
- ETF/레버리지는 한국 시장 연계 섹터 중심으로 선정

👤 팀장 — 알렉스 (Alex, 중립국 스위스 출신)
- 마이크와 지훈의 분석을 종합
- 어느 한쪽에 치우치지 않고 리스크/기회를 균형있게 판단
- 최종 추천 종목과 의견 불일치 시 중재 역할
- 뉴스와 경제지표를 종합한 앞으로의 시장 방향 제시
- 섹터별 투자 매력도 코멘트 포함

출력 형식을 반드시 아래와 같이 작성하세요:

🇺🇸 마이크의 분석
- 미국 추천 종목 (최대 2개): 종목명/티커 | 이유 | 목표가 | 손절가
- ETF/레버리지 추천 (최대 2개): ETF명/티커 | 섹터 | 이유
- 한국 종목 코멘트 (1줄)
...
⚖️ 알렉스의 최종 결정
- 최종 추천 종목: 한국 1~2개 / 미국 1~2개 / ETF 1~2개
- 마이크 vs 지훈 의견 차이 요약 (있을 경우)
- 📈 시장 방향 코멘트: 뉴스/경제지표 기반 향후 1~2주 방향
- 🔥 주목 섹터: 현재 가장 매력적인 섹터 1~2개 이유 포함
- ⚠️ 핵심 리스크 한 줄
...
# ===== Raw 데이터 저장 (Gemini 실패 시 Claude 백업용) =====
def save_raw_data(
    screener_text: str,
    economic_text: str,
    news_text: str,
    ...
    """
    Gemini 실패 시 Raw 데이터를 docs/raw_data.json에 저장
    GitHub Pages를 통해 Claude에서 접근 가능
    """
    ...
    data = {
        ...
        "full_input": "\n\n".join(filter(None, [
            screener_text, economic_text, news_text, etf_text
        ])),
    }
    with open(RAW_DATA_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)
    print(f"  💾 Raw 데이터 저장: {RAW_DATA_FILE}")
...
def analyze_stocks(
    ...
    # 항상 Raw 데이터 저장 (성공/실패 무관)
    save_raw_data(screener_text, economic_text, news_text, etf_text)

    sections = [s for s in [screener_text, economic_text, news_text, etf_text] if s]
    full_input = "\n\n".join(sections)
